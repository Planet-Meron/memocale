...
- (void)viewDidLoad
{
    ...
    self.navigationItem.leftBarButtonItem = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAction target:self action:@selector(back)] autorelease];
...
- (void)configureCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSMutableString *m_string = [NSMutableString stringWithFormat:[managedObject valueForKey:@"memoText"]];
    float x = [[managedObject valueForKey:@"x"] floatValue];
    float y = [[managedObject valueForKey:@"y"] floatValue];
    [m_string appendString:[NSString stringWithFormat:@" x=%3.3f y=%4.3f",x,y]];
    cell.detailTextLabel.text = m_string;
}
...
- (void)insertNewObject
{
    ...
    //[newManagedObject setValue:@"memomemo" forKey:@"memoText"];
